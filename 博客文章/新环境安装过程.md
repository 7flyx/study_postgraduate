...
# 克隆 YDLIDAR SDK 仓库
# 如果没有git，需要 自行安装 git，该项目在github上，可能还需“魔法”
...
~~~nim
# 编译 intelligentcar时 可能需要的ros包如下
# 安装 move_base 和相关包
sudo apt update
...
sudo apt install ros-noetic-joy # 手柄节点
sudo apt install ros-noetic-tf
~~~

~~~nim
# 编译ISSPA项目时，可能所需要的ros包
sudo apt update
...
sudo apt-get install ros-noetic-yocs-virtual-sensor
sudo apt-get install ros-noetic-create-description
sudo apt-get install ros-noetic-turtlebot-bringup
...
~~~nim
# 将ISSPA和intelligentcar这两个项目的所有python脚本添加 可执行权限
 cd ~/ISSPA
find src -name "*.py" -exec chmod +x {} \;

cd ~/intelligentcar
find intelligentcar -name "*.py" -exec chmod +x {} \;
~~~
...
如果在正确配置了上述的rules文件之后，`ll /dev/`命令查询出来显示 <font color = red>myserial -> ttyUSB1, rplidar -> ttyUSB0</font>。端口映射反了，那么重新运行：
...
然后全部拔掉 usb扩展板上的全部 usb接口，==**首先插入 pcb板子与usb扩展板的usb连线！！！首先插入 pcb板子与usb扩展板的usb连线！！！首先插入 pcb板子与usb扩展板的usb连线！！！**== 然后再插入雷达、相机、手柄即可。

原理就是 内核分配端口是根据插入顺序来分配的。
